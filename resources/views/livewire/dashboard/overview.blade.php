<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 md:grid-rows-4 gap-4 min-h-[800px]">
        ...
        <div
            class="md:col-span-2 md:row-span-2 bg-indigo-600 dark:bg-indigo-500 rounded-[2.5rem] p-8 relative overflow-hidden group flex flex-col justify-between shadow-xl shadow-indigo-200 dark:shadow-none">
            <!-- Decorative Circle -->
            <div
                class="absolute -top-12 -right-12 w-48 h-48 bg-white/10 rounded-full blur-3xl group-hover:bg-white/20 transition-all duration-500">
            </div>
        ...
        <div class="md:col-span-2 bg-white dark:bg-slate-900 rounded-[2rem] p-6 border border-slate-100 dark:border-slate-800 shadow-sm relative overflow-hidden group"
            ...
            $watch('darkMode', () => setTimeout(() => initChart(), 100))">
        ...
        <div
            class="bg-white dark:bg-slate-900 rounded-[2rem] p-6 border border-slate-100 dark:border-slate-800 flex flex-col justify-between shadow-sm group">
            <div>
                <div class="flex items-center justify-between mb-4">
                    <div
                        class="w-10 h-10 flex items-center justify-center rounded-2xl {{ $lowStockProducts > 0 ? 'bg-rose-50 text-rose-600' : 'bg-green-50 text-green-600' }}">
        ...
        <div
            ...
            <div class="mt-3 flex -space-x-2">
                @for ($i = 0; $i < 4; $i++)
                    <div
                        class="w-6 h-6 rounded-full bg-slate-200 dark:bg-slate-800 border-2 border-white dark:border-slate-900 flex items-center justify-center text-[8px] font-bold">
                        P{{ $i }}</div>
                @endfor
            </div>
        </div>
            ...
            <div class="flex-1 space-y-6">
                @forelse($recentTransactions as $tx)
                    <div class="relative pl-6 border-l-2 border-white/10">
                        <!-- Dot -->
                        <div
                            class="absolute -left-[7px] top-0 w-3 h-3 bg-indigo-400 rounded-full border-2 border-slate-900">
                        </div>
