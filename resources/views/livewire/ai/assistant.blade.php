<div class="h-full flex flex-col bg-white dark:bg-slate-900 rounded-3xl shadow-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
    <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-800/50">
        <div class="flex items-center space-x-3">
            ...
            <h3 class="text-sm font-bold text-slate-800 dark:text-white">SwiftBill Intelligence</h3>
        </div>
        <div class="flex items-center space-x-1">
            <div class="h-2 w-2 rounded-full bg-green-500 animate-pulse"></div>
            <span class="text-xs font-semibold text-slate-400">Sistem Aktif</span>
        </div>
    </div>
    ...
    <div class="p-6 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800">
        <form wire:submit.prevent="askGemini" class="relative group">
            <input type="text" wire:model="question" placeholder="Analisis data toko atau minta saran..." class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-2xl text-xs py-4 px-6 pr-14 focus:ring-2 focus:ring-indigo-500 shadow-inner" {{ $isLoading ? 'disabled' : '' }}>
        ...
        <p class="text-xs text-slate-400 font-medium mt-4 text-center">Dikembangkan dengan Google Gemini Flash 1.5</p>
    </div>
