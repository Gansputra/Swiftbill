<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] shadow-sm">
        <div>
            <h2 class="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Riwayat Stok</h2>
            <p class="text-xs text-slate-500 font-medium">Logistik Inventaris & Mutasi Stok</p>
        </div>
        <button wire:click="$toggle('showForm')" class="px-8 py-3 bg-indigo-600 text-white rounded-2xl text-xs font-semibold shadow-xl shadow-indigo-500/20 hover:bg-slate-900 transition-all flex items-center gap-2">
            ...
            {{ $showForm ? 'Lihat Riwayat' : 'Tambah Mutasi Stok' }}
        </button>
    </div>
    ...
    @if($showForm)
        <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] p-10 shadow-sm">
            <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-6">Catat Mutasi Stok</h3>
            <form wire:submit.prevent="store" class="space-y-8">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-8">
                        <div class="group">
                            <label class="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-2 px-1">Produk Target</label>
                            ...
                            @error('product_id') <span class="text-xs text-rose-500 font-medium mt-2">{{ $message }}</span> @enderror
                        </div>

                        <div class="grid grid-cols-2 gap-6">
                            <div class="group">
                                <label class="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-2 px-1">Jenis Mutasi</label>
                                <select wire:model="type" class="block w-full bg-slate-50 dark:bg-slate-800 border-none rounded-2xl text-xs font-bold py-4 px-6 focus:ring-4 focus:ring-indigo-500/10 transition-all outline-none appearance-none">
                                    ...
                                    <option value="opname_add">Penyesuaian Opname (Tambah)</option>
                                    <option value="opname_deduct">Penyesuaian Opname (Kurang)</option>
                                </select>
                                @error('type') <span class="text-xs text-rose-500 font-medium mt-2">{{ $message }}</span> @enderror
                            </div>
                            <div class="group">
                                <label class="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-2 px-1">Jumlah Perubahan</label>
                                ...
                                @error('quantity') <span class="text-xs text-rose-500 font-medium mt-2">{{ $message }}</span> @enderror
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-8">
                        <div class="group">
                            <label class="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-2 px-1">Catatan Mutasi</label>
                            <textarea wire:model="notes" class="block w-full bg-slate-50 dark:bg-slate-800 border-none rounded-2xl text-xs font-bold py-4 px-6 focus:ring-4 focus:ring-indigo-500/10 transition-all outline-none" rows="5" placeholder="Alasan penyesuaian stok ini..."></textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-3 pt-6 border-t border-slate-50 dark:border-slate-800">
                    ...
                    <button type="submit" class="px-12 py-4 bg-indigo-600 text-white rounded-2xl text-xs font-semibold shadow-xl shadow-indigo-500/20 hover:bg-slate-900 transition-all">
                        Simpan Mutasi Stok
                    </button>
                </div>
            </form>
    ...
    @else
        <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] shadow-sm overflow-hidden text-sm">
            <div class="p-4 border-b border-slate-50 dark:border-slate-800">
                <input type="text" wire:model.live="searchTerm" placeholder="Cari mutasi stok berdasarkan alasan atau SKU..." class="block w-full bg-slate-50 dark:bg-slate-800 border-none rounded-2xl text-xs font-bold py-3 px-6 focus:ring-4 focus:ring-indigo-500/10 transition-all">
            </div>
                ...
                <table class="w-full text-left border-separate border-spacing-y-2">
                    <thead class="bg-slate-50/50 dark:bg-slate-800/50">
                        <tr>
                            ...
                            <th class="px-8 py-5 text-xs font-semibold text-slate-500 dark:text-slate-400">Produk</th>
                            <th class="px-8 py-5 text-xs font-semibold text-slate-500 dark:text-slate-400">Petugas</th>
                            <th class="px-8 py-5 text-xs font-semibold text-slate-500 dark:text-slate-400 text-right rounded-r-2xl">Jumlah</th>
                        </tr>
                    </thead>
                        ...
                        @forelse($movements as $movement)
                            <tr class="group" wire:key="movement-{{ $movement->id }}">
                                <td class="px-8 py-6 rounded-l-2xl group-hover:bg-slate-50/50 dark:group-hover:bg-slate-800/30 transition-all">
                                    <p class="text-xs font-bold text-slate-900 dark:text-white">{{ $movement->created_at->format('d M, H:i') }}</p>
                                    <span class="px-2 py-0.5 mt-1 inline-block rounded text-xs font-semibold {{ in_array($movement->type, ['purchase', 'opname_add']) ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600' }}">
                                        {{ $movement->type === 'purchase' ? 'Pembelian' : ($movement->type === 'opname_add' ? 'Opname (Tambah)' : ($movement->type === 'opname_deduct' ? 'Opname (Kurang)' : str_replace('_', ' ', $movement->type))) }}
                                    </span>
                                </td>
                                <td class="px-8 py-6 group-hover:bg-slate-50/50 dark:group-hover:bg-slate-800/30 transition-all">
                                    <p class="text-xs font-bold text-slate-900 dark:text-white">{{ $movement->product->name }}</p>
                                    <p class="text-xs text-slate-400 font-medium">{{ $movement->product->sku }}</p>
                                </td>
                                <td class="px-8 py-6 group-hover:bg-slate-50/50 dark:group-hover:bg-slate-800/30 transition-all">
                                    <span class="text-xs font-medium text-slate-500">Oleh {{ $movement->user->name ?? 'Sistem' }}</span>
                                    <p class="text-xs text-slate-400 italic mt-1">{{ $movement->notes }}</p>
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-8 py-32 text-center grayscale opacity-30">
                                    ...
                                    <p class="text-xs font-medium text-slate-400">Belum Ada Catatan Riwayat Stok</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
